...
TEST(assembler_tests, all) 
{
  ...
  "  exit\n",
  27,
  libskiff::types::exec_debug_level_e::EXTREME});
  ...
  "  exit\n",
  2,
  libskiff::types::exec_debug_level_e::MINIMAL});
  ...
  for(auto &tc : tcs) {
    ...
    std::optional<std::unique_ptr<libskiff::binary::executable_c>> loaded_binary = 
      libskiff::binary::load_binary("tmp.test.bin");

    CHECK_TRUE(loaded_binary != std::nullopt);
    CHECK_EQUAL(static_cast<int>(tc.debug_level), static_cast<int>(loaded_binary.value().get()->get_debug_level()));
