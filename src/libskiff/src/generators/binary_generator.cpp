...
uint64_t binary_generator::add_constant(const libskiff::types::constant_type_e type,
                                    const std::vector<uint8_t> data)
{
  ...
  _constant_data.insert(_constant_data.end(), data.begin(),
                        data.end());
...
void binary_generator::set_debug(const libskiff::types::exec_debug_level_e level){ _debug = level; }

std::vector<uint8_t> binary_generator::generate_binary() const
{
  auto encoded_compatibility = libskiff::bytecode::helpers::pack_4(libskiff::binary::compatibility);
  auto encoded_number_of_constants = libskiff::bytecode::helpers::pack_8(_number_of_constants);
  auto encoded_number_of_sections = libskiff::bytecode::helpers::pack_8(_num_sections);
  auto encoded_entry = libskiff::bytecode::helpers::pack_8(_entry);
  auto encoded_number_instructions = libskiff::bytecode::helpers::pack_8(_number_of_instructions);
  ...
  // Section Table
  binary.insert(binary.end(), encoded_number_of_sections.begin(), encoded_number_of_sections.end());
  ...
  binary.insert(binary.end(), encoded_number_instructions.begin(), encoded_number_instructions.end());
