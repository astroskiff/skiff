...
enum class command_e { CREATE = 0, OPEN = 1, CLOSE = 2, WRITE = 3, READ = 4 };
...
void io_disk_c::execute(skiff::types::view_t &view)
{
  ...
  case command_e::CREATE:
    return create(slot, view);
  case command_e::OPEN:
    return open(slot, view);
  case command_e::CLOSE:
    return close(slot, view);
  case command_e::WRITE:
    return write(slot, view);
  case command_e::READ:
    return read(slot, view);
  };
}

void io_disk_c::create(skiff::machine::memory::memory_c *slot,
                       skiff::types::view_t &view)
{
  ...
  // std::cout << "Path source slot : " << file_path_source_slot_id <<
  // std::endl;
  ...
  // std::cout << "Path source offset: " << file_path_source_slot_offset <<
  // std::endl;
  ...
  // Pull the path out of memory - this would be a lot faster if we had a
  // 'slice' function on the memory object
  std::string path;
  for (int i = 0; i < file_path_len; i++) {
    auto [byte_okay, value] =
        path_slot->get_hword(file_path_source_slot_offset++);
    if (!byte_okay) {
      return;
    }
...
void io_disk_c::open(skiff::machine::memory::memory_c *slot,
                     skiff::types::view_t &view)
{
  // std::cout << "OPEN\n";
  auto offset = view.integer_registers[1] + skiff::config::word_size_bytes;
  ...
  view.op_register = static_cast<uint64_t>(
      file->open(static_cast<std::ios_base::openmode>(flags & 0xFF)));
}

void io_disk_c::close(skiff::machine::memory::memory_c *slot,
                      skiff::types::view_t &view)
{
  // std::cout << "CLOSE\n";
...
void io_disk_c::write(skiff::machine::memory::memory_c *slot,
                      skiff::types::view_t &view)
{
  // std::cout << "WRITE\n";
  auto offset = view.integer_registers[1] + skiff::config::word_size_bytes;
  ...
  std::vector<uint8_t> data = ss->get_n_bytes(source_offset, len);

  if (data.empty()) {
    return;
  }

  // std::cout << "Writing : " << reinterpret_cast<const char *>(&data[0]) <<
  // std::endl;
...
void io_disk_c::read(skiff::machine::memory::memory_c *slot,
                     skiff::types::view_t &view)
{
  // std::cout << "READ\n";
  auto offset = view.integer_registers[1] + skiff::config::word_size_bytes;
  ...
  if (!ds->put_n_bytes(data, dest_offset)) {
    return;
  }
